        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="row" style="margin-bottom:10px;">
                    <div class="col-2">
                        <button class="btn btn-primary" onclick="window.location='<?= base_url(); ?>/specific_privilege/add';"><i class="fa fa-plus"></i></button>
                    </div>
                </div>
                <div class="card">
                    <!--PAGING -->
                    ...
                    <div class="card-body table-responsive p-0" style="height: 700px;">
                        <table class="table table-head-fixed text-nowrap table-striped">
                            ...
                            <tbody>
                                <?php
                                $no = $startrow;
                                foreach ($specific_privileges as $specific_privilege) :
                                    $no++;
                                ?>
                                    <tr>
                                        <td>
                                            <a class="btn btn-info btn-sm" href="<?= base_url(); ?>/specific_privilege/edit/<?= $specific_privilege->id; ?>">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <a class="btn btn-danger btn-sm" href="#" onclick="delete_confirmation(<?= $specific_privilege->id; ?>);">
                                                <i class="fas fa-trash"></i>
                                            </a>
                                        </td>
                                        <td><?= $no; ?></td>
                                        <td><?= $specific_privilege->name; ?></td>
                                        <td><?= $specific_privilege->denied_message; ?></td>
                                        <td><?= date("d-m-Y H:i:s", strtotime($specific_privilege->created_at)); ?></td>
                                        <td><?= $specific_privilege->created_by; ?></td>
                                    </tr>
                                <?php endforeach ?>
                            </tbody>
                        </table>
...
<script>
    function delete_confirmation(id) {
        $('#modal_title').html('Delete Specific Privilege');
        $('#modal_message').html('Are you sure want to delete this data?');
        $('#modal_type').attr("class", 'modal-content bg-danger');
        $('#modal_ok_link').attr("href", '<?= base_url(); ?>/specific_privilege/delete/' + id);
        $('#modal-form').modal();
    }
</script>