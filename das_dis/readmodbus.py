...
while(True):
    try:
        ...
        for sensor in sensors:
            ...
            if(value < 0):
                value = -1
            sql = "UPDATE sensor_values SET data = '" + str(value) + "' WHERE sensor_id= '" + str(sensor[0]) + "';"
            mycursor.execute(sql)
            mydb.commit()
            sql = "INSERT INTO sensor_value_histories (sensor_id,data) VALUES ('" + str(sensor[0]) + "','" + str(value) + "');"
            mycursor.execute(sql)
            mydb.commit()
